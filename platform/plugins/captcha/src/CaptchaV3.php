...
class CaptchaV3
{
    ...
    protected string $origin = 'https://www.google.com/recaptcha';
    ...
    public function display(array $attributes = ['action' => 'form'], array $options = ['name' => 'g-recaptcha-response']): ?string
    {
        ...
        $input = '<input type="hidden" name="' . $name . '" id="' . $fieldId . '">
            <script>window.recaptchaInputs = window.recaptchaInputs || [];
            window.recaptchaInputs.push("' . $fieldId . '")</script>';
    ...
    public function initJs($fieldId = null, $action = 'form'): void
    {
        ...
        $html = 'var onloadCallback = function() { grecaptcha.ready(function() {
            window.recaptchaInputs.forEach(function(item) {
                refreshRecaptcha(item); });
            });
        };';

        Theme::asset()
            ->container('after_footer')
            ->writeScript('google-recaptcha-' . $fieldId, $html, ['google-recaptcha'])
